...
public class Parser {
    ...
    public boolean alreadyParsedRule(Map<Integer, Integer> memoization) throws Exception {
        Integer memoI = memoization.get(index());
        if ( memoI == null ) return false;
        int memo = memoI;
        System.out.println("parsed list before at index " + index() + "; skip ahead to token index " + memo + ": " + lookahead.get(memo).text);
        if ( memo == FAILED ) throw new Exception("Previous parse failed");
        seek(memo);
        return true;
    }
